...
class Game:
    ...
    def new(self):
        ...
        self.p1_win =pg.image.load(path.join(img_folder, \
                                                "P1 WIN.png")).convert_alpha()
        self.p2_win = pg.image.load(path.join(img_folder, \
                                                "P2 WIN.png")).convert_alpha()
        ...
        self.powerup_images['gelo'] = pg.image.load(path.join(img_folder,'gelo.png')).convert_alpha()
        ...
        self.player1 = Jogador(self,WIDTH*1/3,self.jogadores_1[self.num1],0)
        self.player2 = Jogador(self,WIDTH*2/3,self.jogadores_2[self.num],1)
        ...
        while self.playing:
            ...
            if self.timer2 >= 10:
                if player1_score > player2_score:
                    self.screen.blit(p1_win, (WIDTH/2, 100))
                if player1_score < player2_score:
                    self.screen.blit(p2_win, (WIDTH/2, 100))
                if player1_score == player2_score:
                    self.screen.blit(self.empate, (WIDTH/2, 100))
            if self.timer2 >= 15:
                if self.playing:
                    self.playing = False
                self.running = False
